...
public class StoryImageController {
    ...
    public String add(@RequestParam(value = "fileNameList") List<String> fileNameList,@RequestParam(value = "storyId") Long storyId, RedirectAttributes redirectAttributes) {
    ...
    public String update(@RequestParam(value = "storyId") Long storyId,@RequestParam(value = "fileNameList") List<String> fileNameList, RedirectAttributes redirectAttributes) {
        Story story = storyService.getStory(storyId);
        //增加新的产品图片
        for (String fileName : fileNameList){
            if (fileName != ""){
